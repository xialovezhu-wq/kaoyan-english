...
7. Perform global normalization and deduplication first. Only when at least two stable remaining candidate shards exist, read `../_shared/native-luna-parallel-contract.md` and assign disjoint shards to read-only `explorer` Luna Max Fast leaves. Each leaf proposes only A/B/C fields, article-specific meanings, source-bound notes and grounding gaps for its IDs; it cannot add candidates, redo mastered/bank decisions, generate cross-shard duplicate conclusions, write files, or invoke/poll the background Luna pipeline. Sol waits at one barrier, rejects wrong-runtime or wrong-hash results, and assembles the unique export.
8. Apply references/abc-priority-rules.md.
9. For each A or B card:
...
10. For every generated example:
...
11. Select old words through references/memory-curve-old-word-selection.md. Prefer due and natural active items; use oldest-fallback only when necessary; never use mastered items.
12. Produce the output and state that dated formal curation is a separate next step.
13. Run completion-receipt, duplicate, source, grounding and zero-write checks.
...
The deterministic `complete-article` command and verified receipt run first and remain terminal-only. After completion succeeds, eligibility is true for cold evidence covered by `english_read_bundle`: exact article sentences, master-bank status for known candidates, needed patterns, and coverage. When eligible, MCP-first is mandatory. Mastered-items, business rules, old-word selection, completion, capture, Luna polling, formal curation, and write-foundation validation remain terminal-only.

Every MCP request must use `schema_version=study-read-mcp.v2` and route context for caller `kaoyan-english-vocab-export`, Skill version `2.0.0`, the installed plugin version, a unique route request ID, one canonical evidence-scope hash, ordered chunk metadata, and `consumed_duplicate_read_count=0`. Deterministically split more than 24 stable IDs; every chunk must bind the same article, generation, and authority fingerprint. Any failed or mismatched chunk invalidates the entire route.

If an older task lacks the tool, record `read_route=terminal_fallback` with reason `tool_snapshot_missing` and immediately run the whole existing evidence route without probing, retrying, or restarting Codex. On a missing field or any other failure, drift, inconsistency, timeout, or limit, discard all MCP chunks and rebind the complete covered scope to one fresh canonical terminal generation. Never mix MCP and fallback evidence.
